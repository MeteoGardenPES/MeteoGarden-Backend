...
def test_identifyPlant_missing_image_returns_400(api_client, identify_url, monkeypatch):
    r = api_client.post(identify_url, format="multipart")
...
def test_identifyPlant_invalid_organ_returns_400(
    api_client, identify_url, image_file, monkeypatch
):
    r = api_client.post(
        identify_url,
        data={"image": image_file, "organs": "root"},
        format="multipart",
    )
    assert r.status_code == 400
    body = r.json()
...
def test_identifyPlant_missing_api_key_returns_500(
    ...
    r = api_client.post(identify_url, data={"image": image_file}, format="multipart")
...
#     api_client, identify_url, image_file, monkeypatch
# ):
#     from api.models import Plant, User, Image
#
#     monkeypatch.setattr(os, "getenv", lambda k: "KEY")
#     monkeypatch.setattr(
#         "api.views.views_identify.requests.post",
...
#             json_payload={
#                 "results": [
#                     {"score": 0.91, "species": {"scientificNameWithoutAuthor": "Rosa canina"}}
#                 ]
#             },
#         ),
#     )
...
#         raise AssertionError("fake_import CALLED")
#
#     monkeypatch.setattr("api.views.views_identify.importPlant", fake_import)
#
#
#
#     r = api_client.post(
#         identify_url,
#         data={"image": image_file, "organs": "leaf"},
#         format="multipart",
#     )
