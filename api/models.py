...
class Inventory(models.Model):
    ...
    def clean(self):
        ...
        for seed in self.seeds:
            if (seed not in AlbumEntry.objects.filter(user=self.user).
                    values_list("plant__scientificName", flat=True)):
                raise (ValidationError
...
class PlantInGarden(models.Model):
    ...
    def clean(self):
        ...
        if (
            ...
            raise ValidationError(
                "The last watered date/time must be after "
                "the planting " "date/time."
            )
...
class AlbumEntry(models.Model):
    ...
    def clean(self):
        if self.discoveryDate > self.user.lastEntry.date():
            raise ValidationError("The discovery date cannot be after "
                                  "the user's last entry date.")
